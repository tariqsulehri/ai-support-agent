...
function ChecklistItem({
  ...
  return (
    <div className="flex gap-3 rounded-md border border-slate-200/80 bg-slate-50/80 p-3">
      <div className={`mt-0.5 flex h-6 w-6 shrink-0 items-center justify-center rounded-full text-xs font-bold ${complete ? 'bg-emerald-500 text-white shadow-sm' : 'bg-amber-100 text-amber-800'}`}>
        {complete ? 'OK' : '!'}
      </div>
      <div>
      ...
      <div className="mx-auto w-full max-w-7xl">
        <header className="mb-5 overflow-hidden rounded-lg border border-white/80 bg-white shadow-[0_20px_60px_rgba(15,23,42,0.10)] ring-1 ring-slate-900/5">
          ...
          <div className="flex flex-wrap items-center justify-between gap-4 px-6 py-5">
            <div>
              <p className="text-xs font-semibold uppercase tracking-wide text-cyan-700">Tenant Settings</p>
              <h1 className="mt-1 text-3xl font-semibold text-slate-950">{detail.tenant.companyName}</h1>
              <p className="mt-2 font-mono text-sm text-slate-500">{detail.tenant.tenantId}</p>
            </div>
            <div className="flex flex-wrap gap-2">
              {session.role === 'platform_admin' && (
                <Link href="/admin/tenants" className="rounded-md border border-slate-200 bg-white px-4 py-2 text-sm font-semibold text-slate-700 shadow-sm hover:bg-slate-50">
                  Tenants
                </Link>
              )}
        ...
        {pageParams?.error && (
          <div className="mb-5 rounded-md border border-rose-200 bg-rose-50 px-4 py-3 text-sm font-semibold text-rose-700 shadow-sm">
            {pageParams.error}
          </div>
        )}
        {pageParams?.saved && (
          <div className="mb-5 rounded-md border border-emerald-200 bg-emerald-50 px-4 py-3 text-sm font-semibold text-emerald-700 shadow-sm">
        ...
        {missingRuntimeMessages.length > 0 && (
          <div className="mb-5 rounded-md border border-amber-200 bg-amber-50 px-4 py-3 text-sm font-semibold text-amber-800 shadow-sm">
