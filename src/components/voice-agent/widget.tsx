  ...
  if (mode === 'inline') {
    ...
    return (
      <div className={`min-h-dvh w-full flex items-end justify-end ${inlinePadding}`}>
        <VoiceAgent tenantId={tenantId} token={token} sessionToken={sessionToken} onClose={closeInline} />
      </div>
    )
  }
    ...
    <>
      ...
      {open && (
        <div
          ...
          `}
        >
          <VoiceAgent tenantId={tenantId} token={token} sessionToken={sessionToken} onClose={() => setOpen(false)} />
        </div>
      )}
